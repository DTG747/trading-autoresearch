...
def run_strategy(df):
    ...
    Changes from iter 31:
    - Add breakeven stop: once trade moves 1.5x ATR in our favor,
      move stop to entry price to eliminate small losers
    """
    # --- Parameters ---
    fast_ema = 9
    slow_ema = 21
    ...
    for i in range(warmup, len(df)):
        ...
        if position is None:
            ...
            if hit_tp or low <= stop_price or (prev_ema_f >= prev_ema_s and ema_f < ema_s):
                if hit_tp:
                    exit_price = tp_price
                elif low <= stop_price:
            ...
            if hit_tp or high >= stop_price or (prev_ema_f <= prev_ema_s and ema_f > ema_s):
                if hit_tp:
                    exit_price = tp_price
