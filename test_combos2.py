...
# Fine-tune around RSI=65-70, TE=110-140
for te in [110, 115, 120, 130, 140, 150]:
    for rsi_u in [65, 68, 70, 72, 75]:
        run_with_params(f"RSI={rsi_u} TE={te}", [
            ("rsi_upper = 70", f"rsi_upper = {rsi_u}"),
            ("trend_ema = 50", f"trend_ema = {te}"),
        ])

# Best so far RSI=65 TE=120, try with ATR trail variations
for atr_t in [3.0, 3.5, 4.0, 4.5]:
    run_with_params(f"RSI=65 TE=120 ATR={atr_t}", [
        ("rsi_upper = 70", "rsi_upper = 65"),
        ("trend_ema = 50", "trend_ema = 120"),
